...
$$\hat{\mu}_{a,n} IPWE = \frac{1}{n} \sum_{i=1}^{n} Y_i \frac{\hat{p}_{n}(A_i|A_i-1;\sigma)}{\hat{p}_{n}(A_i=a|X_i;\alpha)} ..... (2)$$
...
Where Y is the weighted outcome, that is a weighted proportion of negative advertising by the end of campaign and T is the weighted 
number of observed negative advertising observed (4 week) and A is a polls position (1 for losing or 0 for leading).
...
nAx <- with(data.panel.df, tx1 + tx2 + tx3)

outDatUnstab <- data.frame(outcome = data.panel.df$outcome, nAx, wt = unstabWt1$es.max.ATE)
sv1unstab <- svydesign(~1, weights = ~wt, data = outDatUnstab)
fitUnstab <- svyglm(outcome ~ nAx, sv1unstab)
coef(fitUnstab)
confint(fitUnstab)
...
outDatStab <- data.frame(outcome = data.panel.df$outcome, nAx, wt = stabWt1$es.max.ATE)
sv1stab <- svydesign(~1, weights = ~wt, data = outDatStab)
fitStab <- svyglm(outcome ~ nAx, sv1stab)
coef(fitStab)
confint(fitStab)
...
library(stargazer)
stargazer(fitUnstab, fitStab, 
          title = "Polls Position Effect to the Proportion of Negative Ads", 
          column.labels = c("Unstabilized", "Stabilized"))
stargazer(confint(fitStab), type = "text", title = "95% confidence interval of Unstabilized weighted Polls Position Coefficient")
```
Finally, Using a unstabilized weighted regressions that account for time-varying confounders this study found that both of polls position (nAx)  and intercept is statistically significant at p.value <0.01. The coefficient for nAx is 0.061. It means that when polls shows that a candidate is trailing, the proportion of negative Facebook advertisement to a total advertisement is increased by 0.061 or 6.1 percent. There is an evidence that polls position affect the proportion of negative advertisement by a candidate. Positive coefficient means that there is a positive relationship between polls position and decision to go negative.

\begin{table}[!htbp] \centering 
  \caption{Polls Position Effect to the Proportion of Negative Ads} 
  \label{} 
...
\hline \\[-1.8ex] 
Observations & 60 & 60 \\ 
Log Likelihood & 21.912 & 19.531 \\ 
Akaike Inf. Crit. & $-$39.825 & $-$35.062 \\ 
\hline 
\hline \\[-1.8ex] 
...
A similar result is shown when regression using at stabilized weight is performed. Both intercept and polls position(nAx) is statistically significant at p.value<0.01. The coefficient for nAx is 0.059. It means that when polls shows that a candidate is trailing, the proportion of negative Facebook advertisement to a total advertisement is increased by 0.059 or 5.9 percent. There is an evidence that polls position affect the proportion of negative advertisement by a candidate. Positive coefficient means that there is a positive relationship between polls position and decision to go negative.

T.test for both stabilezed and unstabilized weight show a corresponding confidence interval of polls position with $\alpha$ = 0.05. As expected, coefficient in stabilized weighted effect has a slightly wider interval than unstabilized weighted. 

\begin{table}[!htbp] \centering 
  \caption{95 percent confidence interval Weighted Polls Position Effect} 
  \label{} 
...
\hline \\[-1.8ex] 
 & 2.5 \% & 97.5 \% \\ 
\hline \\[-1.8ex] 
nAx (Unstabilized) & $0.024$ & $0.097$ \\ 
nAx (Stabilized) & $0.021$ & $0.097$ \\ 
\hline \\[-1.8ex] 
\end{tabular} 
\end{table} 
...
However, this paper did not find an overwhelming evidence that candidate's party affects the proportion of negative advertisement in Facebook data. The table below shows that when regresion is performed on unstabilized weighted data, 
Without interaction term, candidate party nd polls position only significant at p-value <0.1. However, the model 
fail to get significant p-value when we add interaction term between party and polls position.
```{r message=FALSE, results=FALSE, echo=FALSE, cache=TRUE, error=FALSE}
## using region as covariate
iptw.Ex <- iptw(list(tx1 ~ use0 +  region,
                     tx2 ~ use1 + use0 + tx1 + region,
                     tx3 ~ use2 + use1 + use0 + tx2 + tx1 + region),
                timeInvariant ~ region,
                data = data.panel.df,
                cumulative = FALSE,
                priorTreatment = FALSE,
                verbose = FALSE,
...
sv1unstab <- svydesign(~1, weights = ~wt, data = outDatUnstab)
fitUnstab.2 <- svyglm(outcome ~ nAx+party, sv1unstab)
fitUnstab.2.int <- svyglm(outcome ~ nAx*party, sv1unstab)
summary(fitUnstab.2)
summary(fitUnstab.2.int)
# stabilized weights
numWt <- get.weights.num(iptw.Ex, fitList)
stabWt1 <- unstabWt1 * numWt
...
sv1stab <- svydesign(~1, weights = ~wt, data = outDatStab)
fitStab.2 <- svyglm(outcome ~ nAx+party, sv1stab)
fitStab.2.int <- svyglm(outcome ~ nAx*party, sv1stab)
summary(fitStab.2)
summary(fitStab.2.int)
```

```{r message=FALSE, results=FALSE, echo=FALSE, cache=FALSE, error=FALSE, eval=FALSE}
stargazer(fitUnstab.2, fitUnstab.2.int, fitStab.2, fitStab.2.int,
          title = "Polls position and Party Effect on Proportion of Negative Ads.",
          column.labels = c("Unstabilized","Unstabilized w/ interaction","Stabilized","Stabilized w/ interaction"))
```

Regression with stabilized weighting found that candidate's party was not statistically significant while  
position polls (nAx) also remain significant at p-value<0.05. Similar to the regression with unstabilized
weighting, we found that both polls position and candidate's party is unsignificant predictor when we add
interaction term.

\begin{table}[!htbp] \centering 
  \caption{Polls position and Party Effect on Proportion of Negative Ads.} 
  \label{} 
...
\cline{2-5} 
\\[-1.8ex] & \multicolumn{4}{c}{outcome} \\ 
 & Unstabilized & Unstabilized w/ interaction & Stabilized & Stabilized w/ interaction \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4)\\ 
\hline \\[-1.8ex] 
 nAx & 0.038$^{*}$ & 0.022 & 0.057$^{**}$ & 0.014 \\ 
  & (0.022) & (0.016) & (0.027) & (0.018) \\ 
  & & & & \\ 
 partyR & 0.113$^{*}$ & 0.080 & 0.009 & $-$0.014 \\ 
  & (0.060) & (0.084) & (0.062) & (0.070) \\ 
  & & & & \\ 
 nAx:partyR &  & 0.026 &  & 0.051 \\ 
  &  & (0.037) &  & (0.035) \\ 
  & & & & \\ 
 Constant & 0.130$^{***}$ & 0.141$^{***}$ & 0.141$^{***}$ & 0.148$^{***}$ \\ 
  & (0.020) & (0.015) & (0.014) & (0.014) \\ 
  & & & & \\ 
\hline \\[-1.8ex] 
Observations & 60 & 60 & 60 & 60 \\ 
Log Likelihood & 18.302 & 18.446 & 18.664 & 18.967 \\ 
Akaike Inf. Crit. & $-$30.605 & $-$28.892 & $-$31.328 & $-$29.934 \\ 
\hline 
\hline \\[-1.8ex] 
...
The finding however, might be a reasonable result. We argue that candidate party might not be a significant 
predictor due to lack  of variance in polls position by candidate party. In the three weeks of US presidential
Election in our dataset, polls position in competitive states were mostly dominated by Democrat. In a competitive 
states, Republican was only consistently leading in Ohio and putting a fight on Arizona, Georgia, Iowa, Nebraska, 
and Texas.
\newpage

```{r message=FALSE, results=FALSE, echo=FALSE, cache=FALSE, error=FALSE}
library(tidyverse)
losingposition.byparty <- data.panel.df %>%
  group_by(party) %>%
  summarise(tx1 = mean(tx1),
            tx2 = mean(tx2),
            tx3 = mean(tx3))
```

# Discussion and result
Exploratory analysis using training set shows that there might be a relationship between polls position and negative advertising at the national level. at the start of the campaigning, democrat are inherently more negative than republican. As campaign goes on, however, polls position favor a democrat. This circumtances thus push republican to go more negative. We can see that as time goes by, the proportion of negative advertisemnt by democrat is decreasing while their opponent is increasing. This might illustrate that losing candidate engage in more negative advertising as respond to a polls position, or at the very least going more negative than before.

In confirmatory analysis, the dynamic approach shows evidences that the polling position affects the proportion of negative advertising in a political campaign. The relationship is positive, means that being in a trailing position, the candidate will have a higher proportion of negative advertisement on Facebook than before. Additionally, there is no compelling evidence that candidate party affects the proportion of negative advertisement on Facebook.
...
Secondly, there might be other confounding variables that are not included in the dataset. While the polls position is statistically significant, the model can only explain about 17 percent variation in outcome using unstabilized weighting (or 22 percent with stabilized weighting). One possibile confounder is an election debate. The number of advertisements is increased around the time of the first and third presidential debates. However, we see a declining number of advertisements around the second presidential debate, which were canceled.
\newpage
